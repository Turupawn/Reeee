...
contract MyERC20 is Context, IERC20, IERC20Metadata, Ownable {
    ...
    constructor(address _owner, NameSym memory name_sym, 
            address p_marketing_wallet, address p_donation_wallet, address p_liquidity_wallet,
            uint p_total_supply,
            uint p_marketing_fee, uint p_donation_fee, uint p_liquidity_fee) {
        // Editable
        marketing_wallet = p_marketing_wallet;
        donation_wallet = p_donation_wallet;
        liquidity_wallet = p_liquidity_wallet;
        minTokensBeforeSwap = p_total_supply;    // Off by default
        // End editable
        
        _name = name_sym.name;
        _symbol = name_sym.symbol;

        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0xa5E0829CaCEd8fFDD4De3c43696c57F7D7A678ff);
        pair = IUniswapV2Factory(_uniswapV2Router.factory()).createPair(address(this), _uniswapV2Router.WETH());
        router = _uniswapV2Router;

        _marketingFee.push(p_marketing_fee);
        _marketingFee.push(p_marketing_fee);
        _marketingFee.push(0);

        _donationFee.push(p_donation_fee);
        _donationFee.push(p_donation_fee);
        _donationFee.push(0);

        _liquidityFee.push(p_liquidity_fee);
        _liquidityFee.push(p_liquidity_fee);
        ...
        _mint(_owner, p_total_supply);
        //_mint(msg.sender, p_total_supply);
    }
...
contract Factory
{
    function createToken(address owner, NameSym memory name_sym, 
            address p_marketing_wallet, address p_donation_wallet, address p_liquidity_wallet,
            uint p_total_supply,
            uint p_max_wallet_percentage, uint p_max_tx_percentage,
            uint marketing_fee, uint donation_fee, uint liquidity_fee) public returns(address)
    {
        MyERC20 token_contract = new MyERC20(owner, name_sym, 
            p_marketing_wallet, p_donation_wallet, p_liquidity_wallet,
            p_total_supply,
            marketing_fee, donation_fee, liquidity_fee);
        
        token_contract.setMaxWalletPercentage(p_max_wallet_percentage); // 1%
        token_contract.setMaxTxPercentage(p_max_tx_percentage);         // 0.1%
        token_contract.transferOwnership(owner);
